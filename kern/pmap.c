...
mem_init(void)
{
	...
	boot_map_region(kern_pgdir, 
					...
					PADDR(pages), 
					(PTE_U|PTE_P));
	cprintf("the UPAGES VA :%08x\nthe UPAGES PA:%08x\n", UPAGES, PADDR(pages));
	cprintf("the pages VA: %08x\nthe pages PA: %08x\n", pages, PADDR(pages));
	//////////////////////////////////////////////////////////////////////
	...
	cprintf("the KSTACKTOP VA: %08x\nthe bootstack VA: %08x\nthe bootstack PA: %08x\n", KSTACKTOP, bootstack, PADDR(bootstack));
	//////////////////////////////////////////////////////////////////////
...
{
	assert(pgdir != NULL);
	pde_t *pg_d_entry = NULL;           //a pointer to the page table directory entry
	pte_t *pg_table = NULL;        //a pointer to the page table
	struct PageInfo *pginfo = NULL;  //the pageinfor pointer
	pg_d_entry = &pgdir[PDX(va)];     

	if(*pg_d_entry & PTE_P){
		pg_table = KADDR(PTE_ADDR(*pg_d_entry));
	}
	else{
		if(!create)
		  return NULL;
		...
		if(!pg_table)
		  return NULL;
		*pg_d_entry = PADDR(pg_table) | PTE_P | PTE_U | PTE_W;
		pginfo->pp_ref++;
	}
	return &pg_table[PTX(va)];
	// at first get the page table directory entry and check if the entry is present or not
	// i am going home and will go on tomorrow
	// code here
	// Fill this function in
	return NULL;
}
...
{
	// Fill this function in
	pte_t *pg_t_entry = NULL;
	uintptr_t va_t = va;
	physaddr_t pa_t = pa;
	uint32_t i;

	for(i = 0;i < (size/PGSIZE); i++){
		pg_t_entry = pgdir_walk(pgdir, (void *)va, 1);
		*pg_t_entry = pa | perm | PTE_P;			
		va += PGSIZE;
		pa += PGSIZE;
	}
}
...
page_remove(pde_t *pgdir, void *va)
{
	...
	pte_t *pte = NULL;
	pte_t **pte_store = &pte;
	pg = page_lookup(pgdir, va, pte_store); //store the 
	if(!pg)
	  return;
	if((*pte_store)){
		page_decref(pg);
